...
class TestTUINavigation:
    ...
    async def test_home_to_config_navigation(self, tmp_path: Path, sample_audio_mp3: Path):
        ...
        async with app.run_test() as pilot:
            # Push home screen directly (since button navigation isn't working in this branch)
            app.push_screen("home")
            await pilot.pause()
            
            # Verify we're on the home screen
            assert len(app.screen_stack) == 2  # Default screen + HomeScreen
            assert app.screen_stack[-1].__class__.__name__ == "HomeScreen"

            # Get the home screen instance
            home_screen = app.screen_stack[-1]
            
            # Simulate file selection which should navigate to config
            # HomeScreen automatically navigates to config when a file is selected
            home_screen._select_file(sample_audio_mp3)
            await pilot.pause()
            
            # Verify navigation to config screen was successful
            assert len(app.screen_stack) == 3  # Added ConfigScreen
            assert app.screen_stack[-1].__class__.__name__ == "ConfigScreen"
            
            # Verify the config screen has the selected file
            assert app.state.input_path == sample_audio_mp3
