...
from src.cli import create_parser, main, process_command
...
class TestCLITranscribeCommand:
    ...
    def test_transcribe_command_missing_api_key(self, temp_audio_file):
        ...
        with patch("sys.argv", ["audio-extraction-analysis", *test_args]):
            ...
            with patch("src.cli.commands.transcribe.TranscriptionService") as mock_service_class:
                mock_instance = Mock()
...
class TestCLIProcessCommand:
    ...
    @pytest.mark.asyncio
    async def test_process_command_runs_in_active_event_loop(self, tmp_path) -> None:
        """Ensure process command can run inside existing event loop without nested asyncio.run."""
        video_file = tmp_path / "video.mp4"
        video_file.write_bytes(b"data")

        args = create_parser().parse_args(["process", str(video_file)])
        args.output_dir = str(tmp_path / "out")

        fake_transcript = Mock()
        pipeline_result = {"success": True, "transcript": fake_transcript}
    ...
    def test_verbose_logging_setup(self, temp_video_file):
        ...
        with patch("sys.argv", ["audio-extraction-analysis", *test_args]):
            with patch("src.cli.setup_logging") as mock_setup:
                with patch("src.cli.commands.extract.AudioExtractor"):
                    main()
...
class TestCLIProviderSelection:
    ...
    def test_transcribe_command_with_elevenlabs_provider(
        ...
        with patch("sys.argv", ["audio-extraction-analysis", *test_args]):
            # Config is unused in new transcribe_command
            with patch("src.cli.commands.transcribe.TranscriptionService") as mock_service:
                from datetime import datetime
