...
def test_ingest_rejects_non_http_scheme(download_dir: Path) -> None:
    service = UrlIngestionService(download_dir)

    with pytest.raises(UrlIngestionError):
        service.ingest("file:///etc/passwd")


def test_ingest_rejects_loopback_host(download_dir: Path) -> None:
    service = UrlIngestionService(download_dir)

    with pytest.raises(UrlIngestionError):
        service.ingest("http://127.0.0.1/private")


@patch("src.services.url_ingestion.YoutubeDL")
def test_ingest_rejects_download_outside_dir(mock_ydl: MagicMock, download_dir: Path) -> None:
    service = UrlIngestionService(download_dir)

    outside_path = download_dir.parent / "evil.mp3"
    instance = mock_ydl.return_value.__enter__.return_value
    instance.extract_info.return_value = {"id": "abc", "_filename": str(outside_path)}
...
@patch("src.services.url_ingestion.YoutubeDL")
def test_ingest_sanitizes_download_filename(mock_ydl: MagicMock, download_dir: Path) -> None:
    service = UrlIngestionService(download_dir)

    unsafe_file = download_dir / " __evil?.m4a "
    unsafe_file.parent.mkdir(parents=True, exist_ok=True)
    unsafe_file.write_bytes(b"audio")

    instance = mock_ydl.return_value.__enter__.return_value
    instance.extract_info.return_value = {"id": "abc", "_filename": str(unsafe_file)}
...
@patch("src.services.url_ingestion.AudioExtractor")
@patch("src.services.url_ingestion.YoutubeDL")
def test_ingest_keep_video_returns_source(
    mock_ydl: MagicMock, mock_extractor_cls: MagicMock, download_dir: Path
) -> None:
    service = UrlIngestionService(download_dir, prefer_audio_only=False, keep_video=True)

    video_file = download_dir / "abc.mp4"
    audio_file = download_dir / "abc.mp3"
    video_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    extractor_instance = mock_extractor_cls.return_value
    extractor_instance.extract_audio.return_value = str(audio_file)

    result = service.ingest("https://example.com/video", quality=AudioQuality.HIGH)

    assert result.audio_path == audio_file
    assert result.source_video_path == video_file
    extractor_instance.extract_audio.assert_called_once()


@patch("src.services.url_ingestion.AudioExtractor")
@patch("src.services.url_ingestion.YoutubeDL")
def test_ingest_raises_when_audio_extraction_returns_none(
    mock_ydl: MagicMock, mock_extractor_cls: MagicMock, download_dir: Path
) -> None:
    service = UrlIngestionService(download_dir, prefer_audio_only=False)
    ...
    extractor_instance = mock_extractor_cls.return_value
    extractor_instance.extract_audio.return_value = None

    with pytest.raises(UrlIngestionError, match="Audio extraction returned no path"):
        service.ingest("https://example.com/video")
