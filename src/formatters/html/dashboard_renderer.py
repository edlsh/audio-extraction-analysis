...
def _build_rendered_stages(stage_results: dict[str, Any]) -> list[dict[str, str]]:
    """Build sorted rendered stage list for template."""
    rendered = [
        {
    ...
    rendered.sort(key=lambda item: (item["name"].lower() == "total", item["name"]))
    return rendered


def _build_files_list(files_created: list[Any], output_dir: Path) -> list[dict[str, str]]:
    """Build files list with relative paths."""
    files = []
...
def build_dashboard_context(
    ...
    *,
    ...
    return {
        ...
        "stage_results": _build_rendered_stages(stage_results),
        "files": _build_files_list(files_created, output_dir),
        "raw_json": json.dumps(
            {"transcript": transcript_dict, "pipeline": pipeline_result},
            indent=2,
            default=str,
        ),
