"""Legacy CLI shim forwarding to the new modular CLI when available."""
...
_FORWARDED = False
try:
    from src.cli import main as _new_cli_main
except Exception as exc:  # pragma: no cover - fallback to legacy code below if import fails
    logger.warning(
        "Could not import new CLI main; falling back to legacy implementation: %s", exc
    )
else:
    main = _new_cli_main  # type: ignore[assignment]
    _FORWARDED = True
...
async def process_command(
    ...
    """
    try:
        input_path = _resolve_process_input(args)
...
def _resolve_process_input(args: argparse.Namespace) -> Path | None:
    ...
    if getattr(args, "url", None):
        return _ingest_url_for_process(args)

    return _resolve_local_video(args)


def _ingest_url_for_process(args: argparse.Namespace) -> Path | None:
...
def _resolve_local_video(args: argparse.Namespace) -> Path | None:
